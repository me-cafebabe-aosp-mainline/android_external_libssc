...
G_DEFINE_TYPE_WITH_CODE (SSCSensorGyroscope, ssc_sensor_gyroscope, SSC_TYPE_SENSOR,
			 G_ADD_PRIVATE (SSCSensorGyroscope))

typedef struct {
	SSCSensorGyroscope *sensor;
	gfloat x;
	gfloat y;
	gfloat z;
} SignalContext;
...
{
	SscGyroscopeResponse *msg = NULL;
	SSCSensorGyroscope *sensor = SSC_SENSOR_GYROSCOPE (user_data);
	...
	if (sensor_uid_high == uid_high && sensor_uid_low == uid_low && msg_id == SSC_MSG_REPORT_MEASUREMENT) {
		msg = ssc_gyroscope_response__unpack (NULL, buf->len, (const uint8_t *) buf->data);

		if (msg == NULL) {
			g_warning ("Failed to unpack gyroscope measurement message");
			return;
		}
...
static void
gyroscope_close_ready (SSCSensor *sensor, GAsyncResult *result, gpointer user_data)
{
...
gboolean
ssc_sensor_gyroscope_close_finish (SSCSensorGyroscope *self, GAsyncResult *result, GError **error)
{
	return g_task_propagate_boolean (G_TASK (result), error);
}

void
ssc_sensor_gyroscope_close (SSCSensorGyroscope *self, GCancellable *cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
	GTask *task = NULL;
	SSCClient *client = NULL;
	SSCSensorGyroscopePrivate *priv = NULL;

	g_assert (SSC_SENSOR_CLASS (ssc_sensor_gyroscope_parent_class)->close &&
		  SSC_SENSOR_CLASS (ssc_sensor_gyroscope_parent_class)->close_finish);
	...
	/* Close sensor */
	SSC_SENSOR_CLASS (ssc_sensor_gyroscope_parent_class)->close (SSC_SENSOR (self), cancellable, (GAsyncReadyCallback)gyroscope_close_ready, task);
}
...
ssc_sensor_gyroscope_close_sync (SSCSensorGyroscope *self, GCancellable *cancellable, GError **error)
{
	...
	ssc_sensor_gyroscope_close (self, cancellable, ssc_common_callback_sync_context, &ctx);
...
static void
gyroscope_open_ready (SSCSensor *sensor, GAsyncResult *result, gpointer user_data)
{
	GTask *task = G_TASK (user_data);
	g_autoptr (GError) error = NULL;
...
gboolean
ssc_sensor_gyroscope_open_finish (SSCSensorGyroscope *self, GAsyncResult *result, GError **error)
{
	return g_task_propagate_boolean (G_TASK (result), error);
}

void
ssc_sensor_gyroscope_open (SSCSensorGyroscope *self, GCancellable *cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
	SSCClient *client = NULL;
	SSCSensorGyroscopePrivate *priv = NULL;
	GTask *task = NULL;

	g_assert (SSC_SENSOR_CLASS (ssc_sensor_gyroscope_parent_class)->open &&
		  SSC_SENSOR_CLASS (ssc_sensor_gyroscope_parent_class)->open_finish);

	priv = ssc_sensor_gyroscope_get_instance_private (SSC_SENSOR_GYROSCOPE (self));
	...
	/* Open sensor */
	SSC_SENSOR_CLASS (ssc_sensor_gyroscope_parent_class)->open (SSC_SENSOR (self), cancellable, (GAsyncReadyCallback)gyroscope_open_ready, task);
}
...
ssc_sensor_gyroscope_open_sync (SSCSensorGyroscope *self, GCancellable *cancellable, GError **error)
{
	...
	ssc_common_init_sync_context (&ctx);
	ssc_sensor_gyroscope_open (self, cancellable, ssc_common_callback_sync_context, &ctx);
	ssc_common_wait_sync_context (&ctx);
...
static void
ssc_sensor_gyroscope_class_init (SSCSensorGyroscopeClass *klass)
{
...
static void
ssc_sensor_gyroscope_init (SSCSensorGyroscope *self)
{
}

SSCSensorGyroscope *
ssc_sensor_gyroscope_new_finish (GAsyncResult *result, GError **error)
{
	GObject *sensor;
	GObject *source;
	...
	g_object_unref (source);
	return SSC_SENSOR_GYROSCOPE (sensor);
}

void
ssc_sensor_gyroscope_new (GCancellable *cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
	g_async_initable_new_async (
			SSC_TYPE_SENSOR_GYROSCOPE,
			G_PRIORITY_DEFAULT,
			cancellable,
			callback,
			user_data,
			SSC_SENSOR_DATA_TYPE, "gyro",
			NULL);
}
...
ssc_sensor_gyroscope_new_sync (GCancellable *cancellable, GError **error)
{
	...
	ssc_common_init_sync_context (&ctx);
	ssc_sensor_gyroscope_new (cancellable, ssc_common_callback_sync_context, &ctx);
	ssc_common_wait_sync_context (&ctx);
